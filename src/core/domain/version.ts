...
import { ArtemisError } from "#/shared/result.js";
...
export class Version {
    ...
    bumpPremajor(identifier?: string): Version {
        ...
        if (!newVersion) {
            throw new ArtemisError(
                `Failed to bump premajor version ${this._version}`,
                "VERSION_BUMP_ERROR"
            );
        }
        return new Version(newVersion);
    ...
    bumpPreminor(identifier?: string): Version {
        ...
        if (!newVersion) {
            throw new ArtemisError(
                `Failed to bump preminor version ${this._version}`,
                "VERSION_BUMP_ERROR"
            );
        }
        return new Version(newVersion);
    ...
    bumpPrepatch(identifier?: string): Version {
        ...
        if (!newVersion) {
            throw new ArtemisError(
                `Failed to bump prepatch version ${this._version}`,
                "VERSION_BUMP_ERROR"
            );
        }
        return new Version(newVersion);
