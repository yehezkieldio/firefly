...
/**
 * Runtime validation schemas for feature management.
 */
...
const EnabledFeaturesSetSchema = z
    .instanceof(Set)
    .refine((set) => Array.from(set).every((f) => FeatureNameSchema.safeParse(f).success), {
        message: "All enabled features must be valid non-empty strings",
    });

/**
 * Feature metadata for enhanced feature management.
 */
...
export class FeatureManager {
    ...
    constructor(options?: Pick<OrchestratorOptions, "enabledFeatures" | "featureFlags">) {
    ...
    enable(feature: string): FireflyResult<FeatureManager> {
        return validateWithResult(FeatureNameSchema, feature, "feature").andThen((validFeature) => {
            ...
            Object.assign(newManager, { features: updated, metadata: updatedMetadata });
    ...
    setMetadata(
        ...
        return validateWithResult(FeatureNameSchema, feature, "feature").map((validFeature) => {
            ...
            const newManager = new FeatureManager();
            Object.assign(newManager, { features: this.features, metadata: updatedMetadata });
            return newManager;
        });
    }
    ...
    static fromConfig(config: Record<string, boolean>): FireflyResult<FeatureManager> {
        ...
        return fireflyOk(new FeatureManager({ featureFlags }));
    }
