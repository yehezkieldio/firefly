// okAsync unused: prefer FireflyOkAsync from result constructors for consistency
...
export function withDryRun<T>(
    ...
    if (options?.dryRun) {
        ...
        return FireflyOkAsync(dryRunReturnValue as T);
    }
    return operation();
}
...
export function createDryRunWrapper(prefix: string) {
    return <T>(
        ...
        operation: () => FireflyAsyncResult<T>,
        dryRunReturnValue?: T
    ): FireflyAsyncResult<T> => withDryRun(options, `${prefix}: ${operationName}`, operation, dryRunReturnValue);
}
