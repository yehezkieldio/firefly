 ...
 * Task Helpers Module
 *
 * Provides convenience functions for common task creation patterns.
 * Reduces boilerplate when creating tasks that follow standard patterns.
 *
 * @module task-system/task-helpers
 */
...
export function createValidationTask<TContext extends GenericWorkflowContext = GenericWorkflowContext>(
    ...
    builder = builder.execute((ctx) => {
        const initial: FireflyAsyncResult<void> = okAsync(undefined);
...
export function pipeline<TContext extends GenericWorkflowContext>(
    ...
    return ops.reduce<FireflyAsyncResult<TContext>>((chain, op) => chain.andThen(op), okAsync(ctx));
}
