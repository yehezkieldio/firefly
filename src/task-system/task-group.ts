 ...
 * Task Group Module
 *
 * Defines types and interfaces for organizing related tasks into groups.
 * Task groups provide hierarchical skip logic, group-level dependencies,
 * and task ID namespacing while preserving individual task visibility.
 *
 * @module task-system/task-group
 */
 ...
 *
 * Groups provide:
 * - **Namespace isolation**: Task IDs are prefixed with `group:task`
 * - **Hierarchical skip logic**: Group-level skip propagates to all tasks
 * - **Group dependencies**: Groups can depend on other groups
 * - **Individual task visibility**: Tasks remain visible in execution logs
 *
